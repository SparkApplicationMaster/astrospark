...
public class HealpixBase extends HealpixTables
  ...
  private RangeSet [] queryRingsInternal(Pointing ptg,  double [] radiuses, int fact)
	...
	int bins = radiuses.length;
	if(radiuses == null || bins == 0) {
		return new RangeSet[0];
	}
	RangeSet [] pixsets = new  RangeSet[bins];
	for (int i = 0; i < bins; ++i) {
		pixsets[i] = new RangeSet();
	}
	if (scheme==Scheme.RING)
	{
		int fct=1;
		...
		HealpixBase b2=null;
		double [] rsmall = new double[bins];
		double [] rbig = new double[bins];
		if (fct>1)
		{
			b2=new HealpixBase(fct*nside,Scheme.RING);
			
			for (int my = 0; my < bins; ++my) {
				rsmall[my] = radiuses[my]+b2.maxPixrad();
				rbig[my] = radiuses[my]+maxPixrad();
			}
		}
		else {
			for (int my = 0; my < bins; ++my) {
				rsmall[my] = rbig[my] = inclusive ? radiuses[my]+maxPixrad() : radiuses[my];
			}
		}
		...
		double [] cosrsmall = new double[bins];
  		double [] cosrbig = new double[bins];
  
  		for (int my = 0; my < bins; ++my) {
  			rbig[my] = Math.min (Math.PI,rbig[my]);
  			
  			cosrsmall[my] = FastMath.cos(rsmall[my]);
  			cosrbig[my] = FastMath.cos(rbig[my]);
		 }
  		...
  		double [] rlat1  = new double[bins];
  		double [] zmax = new double[bins];
  		long [] irmin = new long[bins];
  
  		double [] rlat2  = new double[bins];
  		double [] zmin = new double[bins];
  		long [] irmax = new long[bins];
  		
  		for (int my = 0; my < bins; ++my) {
  			rlat1[my]  = ptg.theta - rsmall[my];
  			zmax[my] = FastMath.cos(rlat1[my]);
  			irmin[my] = ringAbove (zmax[my])+1;

  			if ((rlat1[my]<=0) && (irmin[my]>1)) // north pole in the disk
  			{
  				return null;
  			}
  			...
  			if ((fct>1) && (rlat1[my]>0)) irmin[my]=Math.max(1L,irmin[my]-1);
  			
  			rlat2[my]  = ptg.theta + rsmall[my];
  			zmin[my] = FastMath.cos(rlat2[my]);
  			irmax[my] = ringAbove (zmin[my]);
  			
  			if ((rlat2[my]>=Math.PI) && (irmax[my]+1<nl4)) // south pole in the disk
  	  		{
  	  			return null;
  	  		}
  			
  			if ((fct>1) && (rlat2[my]<Math.PI)) irmax[my]=Math.min(nl4-1,irmax[my]+1);
  		}
  		
  		int my = bins - 1;
  		
  		double [] x = new double [bins];
		double [] ysq = new double [bins];
		double [] dphi= new double [bins];
		
		long [] ip_lo = new long [bins];
		long [] ip_hi = new long [bins];
  		
		Arrays.fill(dphi, 0.0);
		Arrays.fill(ip_lo, 1L);
		Arrays.fill(ip_hi, 0L);
		
		//System.out.println(irmin[bins - 2] + ", " + irmax[bins - 2]);
  		for (long iz= irmin[bins - 1]; iz<=irmax[bins - 1]; ++iz) // rings partially in the disk
  		{
  			/*if (wasNorth) {
  			...
  			while (my > 0 && iz >= irmin[my - 1]) {
  				my--;
  			}
  			while (my < bins - 1 && iz > irmax[my + 1]) {
  				my++;
  			}
  			
  			for (int mmy = 0; mmy < bins - my; ++mmy) {
  				x[my + mmy] = (cosrbig[my + mmy]-z*z0)*xa;
  				ysq[my + mmy] = 1-z*z-x[my+mmy]*x[my+mmy];
  	  			dphi[my + mmy] = -1;
  	  			
	  	  		if (ysq[my + mmy] <= 0) // no intersection, ring completely inside or outside
	  				dphi[my + mmy] = (fct==1) ? 0: Math.PI-1e-15;
	  			else
	  				dphi[my + mmy] = FastMath.atan2(Math.sqrt(ysq[my + mmy]),x[my + mmy]);
  			...
  			for (int mmy = 0; mmy < bins - my; ++mmy) {
  				...
  				ip_lo[my + mmy] = (long)Math.floor
  						(nr*Constants.inv_twopi*(ptg.phi-dphi[my + mmy]) - shift)+1;
			...
			long ip_l = ip_lo[my], ip_h = ip_hi[my];
			
			for (int mmy = Math.max(-1, -my); mmy < bins - my - 1; ++mmy) {
				ip_l = ip_lo[my + mmy + 1]; ip_h = ip_hi[my + mmy + 1];
				if (dphi[my + mmy] <= 0) {
					...
					if (ip_l<=ip_h)
		            {
  		            ...
  		            if (ip_l<0)
  		              {
  		              pixsets[my + mmy].append(ipix1,ipix1+ip_h+1);
  		              pixsets[my + mmy].append(ipix1+ip_l+nr,ipix2+1);
  		              }
		            else
		            	pixsets[my + mmy].append(ipix1+ip_l,ipix1+ip_h+1);
		            }
				}
			}
			
  			for (int mmy = 0; mmy < bins - my - 1; ++mmy) {
  				
  				ip_l = ip_lo[my + mmy + 1]; ip_h = ip_lo[my + mmy];
  				...
  				if (ip_l <= ip_h)
  				{
  					...
  					{
  						if (ip_h <= 0) {
							pixsets[my + mmy].add(ipix1+ip_l+nr,ipix1+ip_h+nr);
  						} else {
							pixsets[my + mmy].add(ipix1,ipix1+ip_h);
							pixsets[my + mmy].add(ipix1+ip_l+nr,ipix2+1);
  						}
  					}
  					else {
  						pixsets[my + mmy].add(ipix1+ip_l,ipix1+ip_h);
  					}
  				}
  				
  				ip_l = ip_hi[my + mmy] + 1; ip_h = ip_hi[my + mmy + 1];
  				if (ip_l <= ip_h)
  				{
  					if (ip_h >= nr)
  					{ ip_l-=nr; ip_h-=nr; }
  					...
  					if (ip_l < 0)
  					{
						pixsets[my + mmy].add(ipix1,ipix1+ip_h+1);
						pixsets[my + mmy].add(ipix1+ip_l+nr,ipix2+1);
  					}
  					else {
	  					pixsets[my + mmy].add(ipix1+ip_l,ipix1+ip_h+1);
  					}
  				} 
  			}
	...
	/*else // scheme_==NEST
	{
		if (radiuses>=Math.PI) // disk covers the whole sphere
		{ pixsets.append(0,npix); return pixsets; }
		...
		double cosrad=FastMath.cos(radiuses),
				sinrad=FastMath.sin(radiuses);
	    ...
	    crpdr[o] = (radiuses+dr>Math.PI) ? -1. : cosrad*cdr-sinrad*sdr;
	    crmdr[o] = (radiuses-dr<0.)      ?  1. : cosrad*cdr+sinrad*sdr;
	    }
	...
	/* Still experimental, therefore disabled
	  ...
	  while (stk.size()>0) {// as long as there are pixels on the stack
	    ...
	    if (cangdist>crpdr[o])
	      ...
	      check_pixel (o, omax, zone, pixsets, pix, stk, inclusive);
	      }
	    }
	  }*/
	return pixsets;
  ...
  private RangeSet queryDiscInternal(Pointing ptg, double radius, int fact)
    ...
    if (scheme==Scheme.RING)
      ...
      if ((rlat1<=0) && (irmin>1)) // north pole in the disk
        ...
        //System.out.println("NORTH POLE in radius: " + Math.toDegrees(radius));
        }
