...
public class RangeSet implements Serializable
  ...
  public double convolution(double[] arr) {
	    double res=0.0;
	    for (int i=0; i<sz; i+=2)
	      for (int j=(int) r[i]; j<r[i+1]; ++j)
	        res += arr[j];
	    return res;	
  }

	public double convolution(LongMap<Double> arr) {
	  double res=0.0;
	  for (int i=0; i<sz; i+=2) {
	    for (long j=r[i]; j<r[i+1]; ++j) {
	  	Double x = arr.getOrNull(j);
	      res += (x == null ? 0 : x.intValue());
	    }
	  }
	  return res;	
	}
